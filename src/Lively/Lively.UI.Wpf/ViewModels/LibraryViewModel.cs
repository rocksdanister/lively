...
namespace Lively.UI.Wpf.ViewModels
{
    public class LibraryViewModel : ObservableObject
    {
        ...
        public void WallpaperVideoConvert(object obj)
        {
            /*
            var selection = (ILibraryModel)obj;
            var model = new LibraryModel(selection.LivelyInfo, selection.LivelyInfoFolderPath, LibraryItemType.videoConvert, userSettings.Settings.LivelyGUIRendering == LivelyGUIState.normal);
        ...
        */
        ...
        public void AddWallpaper(string path, WallpaperType wpType, LibraryItemType dataType, IDisplayMonitor screen, string cmdArgs = null)
        {
            var dir = Path.Combine(Program.WallpaperDir, "SaveData", "wptmp", Path.GetRandomFileName());
            if (dataType == LibraryItemType.processing) //||
                //dataType == LibraryItemType.cmdImport ||
                //dataType == LibraryItemType.multiImport)
            {
            ...
            if (File.Exists(Path.Combine(folderPath, "LivelyInfo.json")))
            {
                ...
                if (info != null)
                {
                    if (info.Type == WallpaperType.videostream || info.Type == WallpaperType.url)
                    {
                        ...
                        return new LibraryModel(info, folderPath, LibraryItemType.ready, userSettings.Settings.LivelyGUIRendering == LivelyGUIState.normal);
                    }
                    else
                    {
